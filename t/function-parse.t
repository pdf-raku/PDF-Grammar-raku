...
my $if-ast = ( :expr[ :int(1), :int(1), :op<add>, :int(3), :op<eq>, {if => :expr[ :int(360), :op<mul>, :op<sin>, :int(2), :op<div>, :op<exch>, :int(360), :op<mul>, :op<sin>, :int(2), :op<div>, :op<add>]}]);

my $if-else-expr = '{2 1 1 add eq {7 6 mul} {(booya!)} ifelse}';
my $if-else-ast = ( :expr[:int(2), :int(1), :int(1), :op<add>, :op<eq>, {"if" => :expr[ :int(7), :int(6), :op<mul>], "else" => :expr[ :literal<booya!> ]}]);
