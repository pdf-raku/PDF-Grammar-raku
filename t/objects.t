...
for ('123', '43445', '+17', '-98', '0') {
    ok($_ ~~ /^<PDF::Grammar::Simple::int>$/, "int: $_");
    ok($_ ~~ /^<PDF::Grammar::Simple::real>$/, "real: $_");
    ok($_ ~~ /^<PDF::Grammar::Simple::object>$/, "object: $_");
}
for ('1234567890.', '.0', '34.5', '-3.62', '+123.6', '4.', '-.002', '0.0') {
    ok($_ !~~ /^<PDF::Grammar::Simple::int>$/, "not int: $_");
    ok($_  ~~ /^<PDF::Grammar::Simple::real>$/, "real: $_");
    ok($_ ~~ /^<PDF::Grammar::Simple::object>$/, "object: $_");
}
...
for ('', '123A', '.', '. 42', '16#FFFE', '6.02E23') {
    ok($_ !~~ /^<PDF::Grammar::Simple::int>$/, "not int: $_");
    ok($_ !~~ /^<PDF::Grammar::Simple::real>$/, "not real: $_");
    ok($_ !~~ /^<PDF::Grammar::Simple::object>$/, "not object: $_");
}

# bool
for ('true', 'false') {
    ok($_ ~~ /^<PDF::Grammar::Simple::bool>$/, "bool: $_");
    ok($_ ~~ /^<PDF::Grammar::Simple::object>$/, "object: $_");
}
ok(''           !~~ /^<PDF::Grammar::Simple::bool>$/, 'not bool: blank');

# hex strings
ok('<Af309>'     ~~ /^<PDF::Grammar::Simple::hex_string>$/, 'hex: basic');
ok('<Af309>'     ~~ /^<PDF::Grammar::Simple::object>$/, 'hex object: basic');
...
# -- escaped
for ('\n', '\r', '\t', '\b', '\f', '\(', '\(', '\40', '\040') {
    ok($_      ~~ /^<PDF::Grammar::Simple::literal_char_escaped>$/, "literal char escaped: $_");
    ok($_     !~~ /^<PDF::Grammar::Simple::literal_char_regular>+$/, "not literal char regular: $_");
    ok("($_)"  ~~ /^<PDF::Grammar::Simple::literal_string>$/, "literal string: ($_)");
}
# -- regular
for ('a', '}', ' ') {
    ok($_      ~~ /^<PDF::Grammar::Simple::literal_char_regular>$/, "literal char regular: $_");
    ok($_     !~~ /^<PDF::Grammar::Simple::literal_char_escaped>$/, "not literal char escaped: $_");
    ok($_     !~~ /^<PDF::Grammar::Simple::literal_line_continuation>$/, "not literal line continuation: $_");
    ok("($_)"  ~~ /^<PDF::Grammar::Simple::literal_string>$/, "literal string: ($_)");
}

# [pdf 1.7] section 7.2.4.2 example 5:
# -- '\0053' :== \005 + '3'
ok('\0053' ~~ /^<PDF::Grammar::Simple::literal_char_escaped>3$/, "literal escaped char followed by numeric");
# -- '\005' :== '\05' :== \005
ok('\005' ~~ /^<PDF::Grammar::Simple::literal_char_escaped>$/, "literal escaped 3 octal digits");
ok('\05' ~~ /^<PDF::Grammar::Simple::literal_char_escaped>$/, "literal escaped 2 octal digits");

do {
    # TODO: some octal escaping cases - not clear from spec
    # (check handling by gs, xpdf, acroreader)";
   ok('\5' ~~ /^<PDF::Grammar::Simple::literal_char_escaped>$/, "literal escaped 1 octal digit"); 
   ok('\059' ~~ /^<PDF::Grammar::Simple::literal_char_escaped>9$/, "literal escaped 2 octal digits - followed by non-octal/decimal digit"); 
}

# -- invalid
for ('\99', '\x', '\?', ')') {
    ok($_     !~~ /^<PDF::Grammar::Simple::literal_char_escaped>$/, "not literal escape: $_");
    ok($_     !~~ /^<PDF::Grammar::Simple::literal_char_regular>+$/, "not literal char regular: $_");
    ok($_     !~~ /^<PDF::Grammar::Simple::literal_line_continuation>+$/, "not literal line_continuation: $_");
    ok("($_)" !~~ /^<PDF::Grammar::Simple::literal_string>$/, "not literal string: $_");
}

# -- line continuation
for ("\\\n") {
    ok($_  ~~ /^<PDF::Grammar::Simple::literal_line_continuation>$/, "literal line continuation: $_");
}
...
ok('/Name1' ~~ /^<PDF::Grammar::Simple::object>$/, "object: /Name1");
ok('[/Name1]' ~~ /^<PDF::Grammar::Simple::array>$/, "array: [/Name1]");

ok('/a[/b]/c' ~~ /^<PDF::Grammar::Simple::name><PDF::Grammar::Simple::array><PDF::Grammar::Simple::name>$/, "parse: /a[/b]/c");

my $id_plus_array = '/ID[<81b14aafa313db63dbd6f981e49f94f4>
<81b14aafa313db63dbd6f981e49f94f4>
]';
ok($id_plus_array ~~ /^<PDF::Grammar::Simple::name><PDF::Grammar::Simple::array>$/, "parse: $id_plus_array");

for ('[]', '[ ]', '[42]', '[/Name]', '[42/Name]', '[49 3.14 false (Ralph) /SomeName]') {
    ok($_ ~~ /^<PDF::Grammar::Simple::array>$/, "array: $_");
}
...
# null
for ('null') {
    ok($_ ~~ /^<PDF::Grammar::Simple::null>$/, "null: $_");
    ok($_ ~~ /^<PDF::Grammar::Simple::object>$/, "object: $_");
}
